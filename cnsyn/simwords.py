...
words_id: list = words_tree["words"]
...
def anns(word, topK=10):
    ...
    word_key = [index for (index,value) in enumerate(words_id) if value == word]

    if word_key == []:
        sim_words = []
    else:
        word_emb = words_emb[word_key]
        ind = tree_model.query(word_emb, k=topK, return_distance=False)
